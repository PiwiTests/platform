...
> **Status (2026-06-25):** Findings **1.1**, **1.2**, **3.2**, **3.3**, and the conflict-handling part of **1.3 / 2.2** have been implemented (`oauth.ts`, `login.vue`, `docs/authentication.md`). Remaining open items: **1.4** (PKCE), **1.6** (proxy-aware redirect URI), **2.3** (explicit linking UI), **2.4** (domain/org allowlist). Each finding below is annotated with `[FIXED]` or `[OPEN]`.
...
### 1.3 — MEDIUM: [PARTIAL] Username collision on new OAuth user creation → confusing failure
...
### 1.4 — LOW: [OPEN] No PKCE on the authorization request
...
### 1.6 — LOW: [OPEN] Redirect URI derived from request `Host`
...
### 2.2 — LOW: [PARTIAL] Callback failures funnel to a single generic message
...
### 2.3 — LOW: [OPEN] No account-linking affordance for existing logged-in users
...
### 2.4 — LOW: [OPEN] New OAuth users always get `Role.USER` with no admin signal
