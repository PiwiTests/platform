...
### 1.1 — HIGH: `email` / `emailVerified` columns are never populated for OAuth users
...
### 1.2 — MEDIUM: Account linking keys off `username`, not the `email` column
...
### 1.3 — MEDIUM: Username collision on new OAuth user creation → confusing failure
...
### 1.4 — LOW: No PKCE on the authorization request
...
### 1.5 — LOW: `state` comparison is not constant-time; cookie not host-bound
...
### 1.6 — LOW: Redirect URI derived from request `Host`
...
### 2.1 — MEDIUM: OAuth users see an empty, non-actionable email card
...
### 2.2 — LOW: Callback failures funnel to a single generic message
...
### 2.3 — LOW: No account-linking affordance for existing logged-in users
...
### 2.4 — LOW: New OAuth users always get `Role.USER` with no admin signal
...
### 3.2 — LOW: Docs describe linking "by email" but code links by username
...
### 3.3 — LOW: Undocumented behaviors worth adding
