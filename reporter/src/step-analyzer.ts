export function categorizeStep(title: string): string {
  ...
  if (lower.startsWith('page.goto') || lower.startsWith('page.reload') || lower.startsWith('page.goback') || lower.startsWith('page.goforward')) return 'navigation'
  if (lower.startsWith('locator.click') || lower.startsWith('locator.dblclick') || lower.startsWith('locator.check') || lower.startsWith('locator.uncheck') || lower.startsWith('locator.selectoption') || lower.startsWith('locator.tap')) return 'action'
  if (lower.startsWith('locator.fill') || lower.startsWith('locator.type') || lower.startsWith('locator.press') || lower.startsWith('locator.clear') || lower.startsWith('locator.setinputfiles')) return 'input'
  if (lower.startsWith('expect') || lower.startsWith('locator.expect') || lower.startsWith('page.expect')) return 'assertion'
  if (lower.startsWith('locator.waitfor') || lower.startsWith('page.waitfor') || lower.startsWith('page.waitforloadstate') || lower.startsWith('page.waitforurl')) return 'wait'
...
export function flattenSteps(steps: any[]): FlatStep[] {
  ...
  for (const step of steps) {
    result.push({ title: step.title, duration: step.duration, category: categorizeStep(step.title) })
