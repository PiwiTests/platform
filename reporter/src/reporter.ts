...
class PiwiDashboardReporter {
  ...
  onTestEnd(test: any, result: any): void {
    ...
    switch (result.status) {
      ...
      case 'timedOut': this.timedOutTests++; break
    }
  ...
  async onEnd(result: any): Promise<void> {
    ...
    console.log(`[Piwi Dashboard] Test run completed. Status: ${overallStatus} (Playwright result.status: ${result?.status || 'undefined'})`)
  ...
  private startStreaming(): void {
    ...
    this.streamStartPromise = (async () => {
      try {
        ...
        if (this.options.verbose) console.log(`[Piwi Dashboard] Streaming not available: ${error.message}. Will use batch mode.`)
  ...
  private flushStreamEvents(): void {
    ...
    const promise = this.httpClient.postJSON(
  ...
  private scheduleRetryFlush(): void {
    ...
    if (this.options.verbose) console.log(`[Piwi Dashboard] Will retry streaming flush in ${delay}ms (attempt ${this.retryCount})`)
  ...
  private async drainPendingEvents(): Promise<void> {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      if (this.options.verbose) console.warn(`[Piwi Dashboard] ${this.pendingEvents.length} events pending, retrying (attempt ${attempt + 1}/${MAX_ATTEMPTS})...`)
  ...
  private async tryFinishStreaming(overallStatus: string, duration: number, sessionCookie: string | null): Promise<boolean> {
    try {
      const durations = this.testCases
        .filter((tc: any) => tc.duration != null)
        .map((tc: any) => tc.duration)

      const flakyTests = this.testCases.filter(
        (tc: any) => tc.status === 'passed' && (tc.retries || 0) > 0
      ).length
      ...
      if (this.options.uploadTraces || hasReports) {
        try {
          await this.uploader.uploadFilesForStreamingRun(
            ...
            this.streamingRunId,
            this.testCases,
            { uploadTraces: this.options.uploadTraces, uploadReport: this.options.uploadReport, reports: this.options.reports },
            this.startTime,
            sessionCookie
          )
        } catch (error: any) {
  ...
  private async tryUploadWithFiles(overallStatus: string, duration: number, sessionCookie: string | null): Promise<boolean> {
    try {
      await this.uploader.uploadWithFiles(
        ...
        duration,
        this.startTime,
        { totalTests: this.totalTests, passedTests: this.passedTests, failedTests: this.failedTests, skippedTests: this.skippedTests },
  ...
  private resolveOverallStatus(result: any): string {
    if (result?.status) {
      ...
      if (result.status === 'failed' || result.status === 'timedout' || result.status === 'interrupted') return 'failed'
    } else {
  ...
  private attachPerformanceData(testCase: any, attachments: any[]): void {
    ...
    if (net?.body) { try { testCase.networkRequests = JSON.parse(net.body.toString()) } catch { /* ignore */ } }

    const vitals = find('piwi-dashboard-web-vitals')
    if (vitals?.body) { try { testCase.webVitals = JSON.parse(vitals.body.toString()) } catch { /* ignore */ } }

    const consoleLog = find('piwi-dashboard-console')
    if (consoleLog?.body) { try { testCase.consoleLogs = JSON.parse(consoleLog.body.toString()) } catch { /* ignore */ } }
