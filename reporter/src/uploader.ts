...
export class Uploader {
  ...
  async uploadJSON(
    ...
    counters: { totalTests: number; passedTests: number; failedTests: number; skippedTests: number },
    ...
    const response = await this.httpClient.postJSON('/api/test-runs/submit', {
      projectName,
      projectDescription,
    ...
    if (response.testRunId) console.log(`[Piwi Dashboard] Test Run ID: ${response.testRunId}, Project ID: ${response.projectId}`)
    return response
  }
  ...
  async uploadWithFiles(
    ...
    projectDescription: string | undefined,
    testCases: any[],
    options: { uploadTraces?: boolean; uploadReport?: boolean; reports?: Array<{ type: string; dir?: string; label?: string }> },
    auth: string | null
  ): Promise<any> {
  ...
  async uploadFilesForStreamingRun(
    projectName: string,
    runId: number,
    testCases: any[],
    options: { uploadTraces?: boolean; uploadReport?: boolean; reports?: Array<{ type: string; dir?: string; label?: string }> },
    ...
    const traceHashMap = options.uploadTraces ? await this.fileHandler.computeTraceHashes(testCases) : new Map()
    const missingHashes = options.uploadTraces
      ? await this.fileHandler.checkMissingTraces(this.httpClient, projectName, traceHashMap, auth)
      : new Set<string>()
    ...
    await this.appendReportsToForm(form, options.reports, options.uploadReport)
    await this.appendTracesToForm(form, testCases, options.uploadTraces, missingHashes, traceHashMap)

    const response = await this.httpClient.postFormData('/api/test-runs/upload', form, auth)
    console.log(`[Piwi Dashboard] Successfully uploaded files for streaming run #${runId}`)
    if (response.reports) {
      for (const r of response.reports) console.log(`[Piwi Dashboard] ${r.label}: ${r.path}`)
    }
  }
  ...
  private async appendTracesToForm(
    form: FormData,
    testCases: any[],
    uploadTraces?: boolean,
    missingHashes?: Set<string> | null,
    traceHashMap?: Map<number, { tracePath: string; hash: string; size: number }> | null
  ): Promise<void> {
    ...
    let uploadCount = 0
    let dedupCount = 0
    const hashesObj: Record<number, string> = {}

    for (const [i, info] of traceHashMap.entries()) {
      hashesObj[i] = info.hash
      if (!missingHashes || missingHashes.has(info.hash)) {
        form.append(`trace_${i}`, fs.createReadStream(info.tracePath), { filename: path.basename(info.tracePath) })
        uploadCount++
      } else {
        dedupCount++
      }
    ...
    console.log(`[Piwi Dashboard] Uploading ${uploadCount} trace files (${dedupCount} already stored, skipped)`)
  }
}
