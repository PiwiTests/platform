...
export function generateFixPrompt(tc: FixPromptInput): string {
  ...
  const lines: string[] = [];
  lines.push(
    `My Playwright test "${title}" ${status === 'failed' || status === 'timedOut' ? 'failed' : 'had issues'} at ${location}.`,
  );
  lines.push('');
  ...
  if (tc.cluster) {
    if (tc.cluster.sameRunCaseCount > 1) {
      lines.push(
        `${tc.cluster.sameRunCaseCount} tests in this run failed with this same normalized error — likely one shared root cause.`,
      );
    }
    lines.push(
      tc.cluster.isNew
  ...
  lines.push('');
  lines.push(
    'What went wrong and how do I fix it? If the root cause is not obvious from the evidence, list multiple possible hypotheses ranked by likelihood, with reasoning for each.',
  );
  lines.push('');
