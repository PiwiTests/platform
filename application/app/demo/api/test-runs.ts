...
function computeMetaDiffDemo(prevMeta: any, currMeta: any, prevEnv: string | null, currEnv: string | null) {
  ...
  if (prevBrowsers !== currBrowsers) diff.push({ key: 'browsers', label: 'Browsers', before: prevBrowsers || null, after: currBrowsers || null })
  return diff
}
...
export async function apiGetRegressionContext(id: number) {
  ...
  const allProjectRuns = await db.select({
    id: testRuns.id,
    startTime: testRuns.startTime,
    status: testRuns.status,
    environment: testRuns.environment,
    metadata: testRuns.metadata
  }).from(testRuns).where(eq(testRuns.projectId, run.projectId))
  ...
  const greenRuns = allProjectRuns
    ...
    .sort((a, b) => new Date(b.startTime as string | Date).getTime() - new Date(a.startTime as string | Date).getTime())
