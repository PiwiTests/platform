...
import { sanitizeMetadata, sanitizeNetworkRequests, sanitizeWebVitals, sanitizeConsoleLogs } from '~~/server/utils/sanitize'
...
export async function apiPostRunEvents(
  ...
  const insertedStatusCounts = cases.slice(0, insertedCount).reduce((acc: Record<string, number>, tc) => {
