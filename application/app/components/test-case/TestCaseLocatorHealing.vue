...
/**
 * Displays ranked alternative locator suggestions for a failing locator.
 * Used on the test case detail page below the error card.
 */
...
const sourceLabel = computed(() => {
  switch (healing.value?.source) {
    case 'prior-run':
      return 'From last passing run';
    case 'fingerprint':
      return 'From prior run (by locator signature)';
    case 'aria-snapshot':
      return 'From failure-time ARIA snapshot';
    default:
      return '';
  }
});
...
<template>
  ...
  >
    <div class="space-y-3">
      <!-- Failing locator -->
      <div v-if="healing?.failingLocator" class="space-y-1">
        <p class="text-xs text-gray-500">Failing locator:</p>
        <div class="flex items-center gap-2">
          <code class="text-xs font-mono bg-elevated rounded px-2 py-1 text-red-600 dark:text-red-400 flex-1 truncate">
          ...
          <UButton
            ...
            @click="
              copy(healing.failingLocator.method + '(' + JSON.stringify(healing.failingLocator.args) + ')', {
                toast: 'Locator copied',
              })
            "
          />
        </div>
      ...
      <div class="space-y-2">
        ...
        >
          <UBadge size="sm" :color="scoreColor(alt.score)" variant="subtle" class="shrink-0 w-10 text-center font-mono">
            {{ alt.score }}
          </UBadge>
  ...
  <!-- Aria snapshot fallback notice -->
  <SectionCard
    v-else-if="healing?.source === 'aria-snapshot'"
    icon="i-lucide-bandage"
    title="Alternative locators"
    subtitle="Limited alternatives — from ARIA snapshot only"
    help="locator-healing"
  >
    <UAlert
      color="info"
      icon="i-lucide-info"
      variant="subtle"
      description="No HTML attributes or data-testid were available. Enable Piwi fixture capture for full alternatives including data-testid, CSS, and XPath."
    />
  </SectionCard>
