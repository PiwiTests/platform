  ...
  <UDashboardPanel id="test-run-detail">
    ...
    <template #body>
      <div class="p-3 lg:p-4 space-y-4">
        <!-- ===== TOP ROW: summary (most width) + 3 metadata blocks (1/6 each) ===== -->
        <div class="grid grid-cols-1 lg:grid-cols-8 gap-4">
          <!-- Summary card - takes 5/8 = most width -->
                ...
                <div>
                  ...
                  <div v-if="isLive && displayProgress" class="mt-2">
                    ...
                    <div class="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden flex">
                      <div v-if="displayProgress.passedTests > 0" class="h-full bg-green-500 transition-all duration-500" :style="{ width: `${(displayProgress.passedTests / Math.max(displayProgress.totalTests || displayProgress.passedTests + displayProgress.failedTests + displayProgress.skippedTests, 1)) * 100}%` }" />
                      <div v-if="displayProgress.failedTests > 0" class="h-full bg-red-500 transition-all duration-500" :style="{ width: `${(displayProgress.failedTests / Math.max(displayProgress.totalTests || displayProgress.passedTests + displayProgress.failedTests + displayProgress.skippedTests, 1)) * 100}%` }" />
                      <div v-if="displayProgress.skippedTests > 0" class="h-full bg-gray-400 transition-all duration-500" :style="{ width: `${(displayProgress.skippedTests / Math.max(displayProgress.totalTests || displayProgress.passedTests + displayProgress.failedTests + displayProgress.skippedTests, 1)) * 100}%` }" />
                    </div>
                  </div>
                </div>
