...
async function getFirstCaseTraces(
  request: APIRequestContext,
  testRunId: number
) {
  ...
  return {
    traces: await (await request.get(`/api/test-cases/${tc.id}/traces`)).json() as Array<{ id: number, filePath: string }>,
    testCase: tc
  }
}
