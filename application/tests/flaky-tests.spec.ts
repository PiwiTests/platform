...
async function submitRun(
  ...
  cases: Array<{ title: string, status: string, duration?: number, location?: string, error?: string, retries?: number }>,
  startTime?: string
) {
...
test.describe.serial('Flaky tests endpoint', () => {
  ...
  test('detects flaky test via retry-pass pattern over multiple runs', async ({ request }) => {
    ...
    for (let i = 0; i < 5; i++) {
      const isFlaky = i < 3
      await submitRun(request, isFlaky
        ? [
            // Fail on first attempt, pass on retry
            { title: 'login flaky test', status: 'failed', duration: 1500, location: 'tests/auth.spec.ts:1:1',
              error: 'TimeoutError: 30000ms\n    at tests/auth.spec.ts:1', retries: 0 },
            { title: 'login flaky test', status: 'passed', duration: 1200, location: 'tests/auth.spec.ts:1:1', retries: 1 },
            { title: 'stable test', status: 'passed', duration: 500, location: 'tests/a.spec.ts:1:1' }
          ]
        : [
  ...
  test('detects flaky test via alternating pass/fail pattern (no retries)', async ({ request }) => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      await submitRun(request, [
        fail
          ? { title: 'alternating test', status: 'failed', duration: 800, location: 'tests/b.spec.ts:1:1',
