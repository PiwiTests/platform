...
export default eventHandler(async (event) => {
  ...
  const recentRuns = await db.select({ id: testRuns.id, startTime: testRuns.startTime })
    .from(testRuns)
  ...
  const runsWithStatus = await db.select({ id: testRuns.id, startTime: testRuns.startTime, status: testRuns.status })
    .from(testRuns)
