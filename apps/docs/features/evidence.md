...
- **Evidence** — one card with content-level tabs; a tab shows a count or a dot when it holds data. A fixture-backed tab a project never captured is removed rather than shown empty (see [Why a card is empty](#why-a-card-is-empty)). The tab *is* the heading — no block inside repeats it. The card **opens on the story**: the tab the story's leading clue cites (when it is strong or medium), else the **Timeline** when it can place two or more items, else **Screen** when a screenshot or video exists, else **Source** — never **State**, and a weak clue never picks. A clue or diagnosis citation switches tabs for you:
  - **Timeline** — one time axis for the **steps**, **console** entries, **network** requests, their **backend logs** and (Playwright 1.63+) open **browser dialogs**, with the **moment of failure** marked: *what the app was doing when the test gave up*. Below, **one steps table** gives each step's offset from the failure (`t-1.1s`), category, label (the failed step in red with its error) and duration bar; the label is the step title, then its target (the rendered locator or the navigation URL) muted; curated **params** fold into a **Parameters** disclosure, locator first; the window's network, console and backend items interleave as rows in time order. **Around the failure** / **Whole test** and the **type chips** drive both axis and table: a chip per item type in the window, with its count, keys its lane; a click hides or shows it, **Only ▾** or Alt-click shows just one, and the failing step and the last shown type always stay. A line names what is hidden, calling out failures — `Hidden: 4 requests (1 failed) · Show all` — and the choice is remembered in this browser. A passing execution shows only the steps table, without offsets.
  ...
  - **Screen** — the failure screenshot (with the **visual diff** and **page diff** toggles), the video, and the trace and attachments; the raw page structure folds behind a **Page structure ▸** disclosure at the bottom, holding the failure-time **ARIA tree** and reconstructed **DOM**, **rendered as the page** from the trace (stylesheets and images embedded, scripts never run, nothing fetched), with **Open in picker** and **Copy HTML**.
...
A project that never captured fixtures gets no dimmed tabs: the **Network**, **Console**, **State** and **Performance** tabs appear only when they hold data (or the fixtures ran and found nothing), and otherwise leave the strip while one footer line names them, with the decline controls for an administrator. Data always wins: the moment fixtures arrive the tabs return.

When a card does open empty, it states exactly one of these:

- **Captured, nothing happened** — the fixtures were active and this run produced nothing. Working as intended.
- **Not applicable** — the card needs a capability the app under test does not have. Backend logs are the case: they need a [Piwi backend integration](/guide/backend-logs).
- **Declined** — the capability was declined for this project; the card names the decision rather than offering to switch it on.

The AI diagnosis reads the same map: its **Data Coverage** block names each absent section the same way.

### Declining a capability

The footer's *Not for this project* and *Not for this instance* **decline** the capture fixtures, so the dashboard stops showing them everywhere. Administrator-only and never final — see [Declining a capability](/guide/getting-started#declining-a-capability).
