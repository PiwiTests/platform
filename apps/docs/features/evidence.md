...
A project that never captured fixtures does not get four dimmed tabs on every execution. The **Network**, **Console**, **State** and **Performance** tabs appear only when they have data (or the fixtures ran and found nothing); when a project has never captured them, they are removed from the strip and one footer line names the missing sources instead — *"Network, console, state and performance are not captured for this project"* — with, for an administrator, *Add fixtures*, *Not for this project* and *Not for this instance*. Data always wins: the moment fixtures start arriving the tabs return on their own.

When a card does open with nothing in it, it states exactly one of these, so a blank block is never ambiguous:
...
- **Declined** — the capability was declined for this project (see [Declining a capability](#declining-a-capability)); the card names the decision rather than offering to switch it on.
...
Not every team uses every capability, and a team that has decided against one should stop meeting it on every page. Any optional capability can be **declined** — at project level (*Not for this project*) or instance-wide (*Not for this instance*) — and the dashboard then hides it everywhere: its evidence tabs, its empty panels, its settings page, its sidebar entry and its project actions all go, rather than lingering half-visible. Declining is administrator-only; everyone else sees the effect (the hidden tabs) and at most the one naming line.

Declining is never final. Every declined capability stays on the in-app [`/setup`](/guide/getting-started) ladder in a folded **Declined** group, each with a *Reconsider* link that clears the decision. A project override can also re-enable a capability the instance declined, from the project's edit form. And because data always wins, a declined capability that starts receiving data reads as active again, with the stored decision shown next to it on the ladder so you can clear it.
