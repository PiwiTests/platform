...
/** An email document: `title` (already escaped) in its head, `body` in the card, `siteUrl` in the footer. */
export function emailLayout(title: string, body: string, siteUrl: string): string {
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1);">
        ...
        <tr><td style="padding:16px 32px;background:#f4f4f5;font-size:12px;color:#71717a;text-align:center;">
          This is an automated message from <a href="${site}" style="color:#18181b;">${site}</a>
        </td></tr>
      </table>
    </td></tr>
  </table>
