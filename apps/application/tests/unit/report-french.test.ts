...
describe('French typography', () => {
  ...
  test('a French email is framed in French', () => {
    const html = emailLayout('t', 'b', 'https://piwi.example', 'fr');
    expect(html).toContain('<html lang="fr">');
    expect(html).toContain('Message automatique envoyé par');
    expect(emailLayout('t', 'b', 'https://piwi.example')).toContain('This is an automated message from');
  });
});

/** Each detector's English title and its French wording. */
const GAP_TITLES: Array<[detector: string, english: string, french: string]> = [
  [
    'success-only',
...
describe('French gap titles', () => {
  test.each(GAP_TITLES)('%s: %s', (detector, english, french) => {
  ...
  test('every detector that titles a gap has its French wording', () => {
    const source = readFileSync(
      fileURLToPath(new URL('../../shared/handlers/scenario-gaps.ts', import.meta.url)),
      'utf8',
    );
    const detectors = new Set([...source.matchAll(/detector: '([a-z-]+)'/g)].map((m) => m[1]!));
    // A prediction handed to locator healing, never a gap.
    detectors.delete('locator-break-ahead');
    expect([...detectors].filter((d) => !GAP_TITLES.some(([id]) => id === d))).toEqual([]);
  });
});
