...
export const HELP_TOPICS = {
  ...
  'case.timeline': {
    ...
    text: 'One time axis that places this execution’s steps, console entries, network requests and backend log entries on the same clock, with a marker at the moment of failure. The default view is the window around the failed step (10s before, 2s after); switch to “Whole test” to see everything. The type chips hide or show steps, requests, console entries, dialogs and backend logs (Alt-click shows only one type; the failing step always stays), and the line beside them says what is hidden; the choice is remembered in this browser. The list below reads it chronologically — click a line to jump to that step, console entry or request. When a run’s reporter recorded no step start times, positions are estimated from durations and the card says so.',
    doc: 'features/evidence#one-execution-diagnosis-first',
  },
  'case.web-vitals': {
    title: 'Web Vitals',
