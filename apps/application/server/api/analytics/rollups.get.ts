...
defineRouteMeta({
  openAPI: {
    ...
    description: `Streams the daily rollup rows of the caller's projects, for a BI tool (Power BI, Metabase, a spreadsheet): one row per project, UTC day, environment, branch and run kind, with the retained and archived parts summed, so the history retention deleted is included. Takes the scope keys of \`GET /api/analytics/[widget]\` (\`period\`, \`projects\`, \`projectTags\`, \`environments\`, \`branches\`, \`allBranches\`, \`fullRunsOnly\`); test filters do not apply, since rollups count runs. Columns: ${ROLLUP_EXPORT_COLUMNS.join(', ')}. The \`*SumMs\` columns are sums over the cell's runs: divide by \`runs\` for an average. \`maxTotalTests\` is the largest suite of one run, never summed. CSV cells that could run as a formula are prefixed with \`'\`.`,
    parameters: [
      {
        name: 'format',
        in: 'query',
