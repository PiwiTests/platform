...
- `reporter/src/locator-healing.ts` — Pure functions for locator capture: alternative generation + stability scoring, ARIA role resolution, the wrapped-method/action/chain constant lists shared with `fixtures.ts`, `captureCallerLocation` (stamps each snapshot's `file:line:col`), and `suggestLocatorsFromAria` (runtime rename hint). No Playwright dependency; structurally mirrors `application/shared/locator-healing.types.ts`
...
| `src/locator-healing.ts`      | Locator capture: alternatives, scoring, call-site stamping |
