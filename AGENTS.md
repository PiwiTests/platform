...
- AI diagnosis env vars (all optional — can also be set via Settings UI):
  ...
  - `PIWI_AI_RESEARCH_MODEL` — optional cheaper/faster model (same provider/key/baseUrl) for a pre-analysis pass before the main model writes the final diagnosis; empty → single-stage. Two-stage breakdown is stored in `failure_diagnoses.details.pipeline` and shown as a "2-stage" badge in the UI.
